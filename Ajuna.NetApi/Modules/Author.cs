...
namespace Ajuna.NetApi.Modules
{
    ...
    public class Author
    {
        ...
        ///   <br />
        /// </returns>
        public async Task<Hash> SubmitExtrinsicAsync(Method method, Account account, ChargePaymentShell chargePaymentShell, uint lifeTime)
        {
            var extrinsic = await _client.GetExtrinsicParametersAsync(method, account, chargePaymentShell, lifeTime, signed: true, CancellationToken.None);

            return await SubmitExtrinsicAsync(Utils.Bytes2HexString(extrinsic.Encode()));
        }

        public async Task<Hash> SubmitExtrinsicAsync(Method method, Account account, ChargePaymentShell chargePaymentShell, uint lifeTime, CancellationToken token)
        {
            var extrinsic = await _client.GetExtrinsicParametersAsync(method, account, chargePaymentShell, lifeTime, signed: true, token);
        ...
        public async Task<string> SubmitAndWatchExtrinsicAsync(Action<string, ExtrinsicStatus> callback,
            Method method, Account account, ChargePaymentShell chargePaymentShell, uint lifeTime)
        {
            var extrinsic = await _client.GetExtrinsicParametersAsync(method, account, chargePaymentShell, lifeTime, signed: true, CancellationToken.None);
        ...
        /// </returns>
        public async Task<string> SubmitAndWatchExtrinsicAsync(Action<string, ExtrinsicStatus> callback, 
            Method method, Account account, ChargePaymentShell chargePaymentShell, uint lifeTime, CancellationToken token)
        {
            var extrinsic = await _client.GetExtrinsicParametersAsync(method, account, chargePaymentShell, lifeTime, signed: true, token);
